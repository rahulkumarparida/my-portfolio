...
// import BackgroundSound from "../components/utils/BackgroundSound";
// ..
AOS.init();
...
const Hero = () => {
  ...
  const pages = ["Home","About Me", "Projects", "Tech Stack", "Certifications", "Github"];
  ...
  return (
    ...
    <div className="min-h-screen w-screen bg-[var(--bg1)] text-[var(--text1)] relative overflow-hidden ">
      {/* <BackgroundSound/> */}
      {/* Navbar */}
      <nav className="fixed top-4 left-1/2 transform -translate-x-1/2 bg-[var(--bg2)] backdrop-blur-2xl px-4 md:px-8 py-3 rounded-full shadow-lg z-50 flex items-center justify-between sm:w-[95%] md:w-[70%]  border border-white/10 backdrop-blur-sm" data-aos="fade-down" data-aos-duration="500"
>
        <div className="flex items-center pr-20 gap-2">
          <img
            ...
            loading="lazy"
          />
          <span className="font-semibold text-base md:text-lg tracking-wide">Rahul</span>
        </div>
        ...
        <ul className="hidden xl:flex  gap-4 lg:gap-6 text-[var(--text2)]">
          {pages.map((p) => (
            ...
            >
              <Link to={p} smooth={true} duration={500} onClick={() => setCurrentPage(p)}>{p}</Link>
            </li>
          ))}
        </ul>
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="hidden xl:block bg-white/10 text-white border border-white/20 px-4 py-1 rounded-full hover:bg-white/20 transition text-xs lg:text-sm md:text-base">
            Let's Connect
          </button>
                  ...
                  >
                    ...
                    <div className="flex flex-col" >
                      {pages.map((p) => (
                        ...
                        >
                          <Link to={p} smooth={true} duration={500} onClick={() => handlePageClick(p)}>{p}</Link>
                        </button>
                      ))}
                    </div>
      ...
      <div className="flex flex-col md:flex-row  items-center justify-center text-center w-full min-h-screen px-4 md:px-6 pt-20 pb-8 md:py-0 md:mt-5 ">
        <div className="img">
            <img src={chibi} data-aos="fade-up" data-aos-duration="1000"
 alt="" className="md:h-120" loading="lazy" />
        </div>

        <div>
        <div className="head w-full max-w-4xl">    
          <motion.h1
